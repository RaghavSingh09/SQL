...
ALTER TABLE GPM_WT_GDI ALTER COLUMN GDI_Name NVARCHAR(500)
GO
ALTER TABLE GPM_WT_GDI ALTER COLUMN Project_Summary NVARCHAR(4000)
GO
ALTER TABLE GPM_WT_GDI ALTER COLUMN Project_Metric_Cp NVARCHAR(4000)
GO

/****** Object:  StoredProcedure [dbo].[Sp_AddWTGDIDetails]    Script Date: 6/29/2019 12:41:00 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[Sp_AddWTGDIDetails]
(
	@vGDI_Name nvarchar(500),
...
DECLARE @_vGDI_Number VARCHAR(15)=NULL
DECLARE @_vGDI_Number_SEQ INT =NULL
DECLARE @_vGDI_Id INT
DECLARE @_vWT_Type Varchar(5)='GDI'
...
DECLARE @_vDvCnt INT=0, @_DvCntMax INT=0
		...
		IF NOT EXISTS(SELECT * FROM GPM_WT_GDI)
			   SELECT @_vGDI_Number=@_vWT_Type+'-00001'
		ELSE
		BEGIN
			   SELECT @_vGDI_Number_SEQ=MAX(CAST(SUBSTRING(GDI_Number,CHARINDEX('-',GDI_Number,1)+1,LEN(GDI_Number)) AS INT))
		FROM GPM_WT_GDI WHERE CHARINDEX('-',GDI_Number,1)>0 AND UPPER(SUBSTRING(GDI_Number,1, CHARINDEX('-',GDI_Number,1)-1))=UPPER(@_vWT_Type)
		AND ISNUMERIC(SUBSTRING(GDI_Number,CHARINDEX('-',GDI_Number,1)+1,LEN(GDI_Number)) )=1

			   IF(@_vGDI_Number_SEQ IS NULL)
					  SELECT @_vGDI_Number=@_vWT_Type+'-00001'
			   ELSE
					  BEGIN
							 SELECT @_vGDI_Number_SEQ=@_vGDI_Number_SEQ+1

							 IF(@_vGDI_Number_SEQ>99999)
							 PRINT 'Sequence Reached To Maximum Limit'
							 ELSE
								   SELECT @_vGDI_Number= @_vWT_Type+'-'+REPLICATE('0', 5-LEN(CAST(@_vGDI_Number_SEQ AS VARCHAR(10))) ) + CAST(@_vGDI_Number_SEQ AS VARCHAR(10))
					  END
...
BEGIN TRAN
         ...
         (
		   ...
		   @_vGDI_Id_REP
		    ...
		    IF (@@ERROR <> 0) GOTO ERR_HANDLER
					...
					SELECT @vGDI_Number=@_vGDI_Number
			...
			WHERE WT_Project_Id=@vWT_Project_Id 
					...
					INSERT INTO GPM_WT_Project_Notification
								...
								FOR XML PATH('')
									),2,100000) +' for project '+ @vGDI_Name,
								@vGDI_Name,
								GETDATE(),
								'N'
			...
			*/
					...
					BEGIN
						INSERT INTO GPM_WT_Project_Notification
								...
								SELECT DISTINCT @vWT_Project_Id,
									...
									GWPT.GD_User_Id,
									@vLast_Modified_By,
									'IDEA '+GWI.Idea_Name+' has been converted into Project '+@vGDI_Name,
									@vGDI_Name,
									GETDATE(),
									'N'
									FROM GPM_WT_Idea GWI INNER JOIN GPM_WT_Project GWP On GWI.Idea_Id=GWP.WT_Id AND GWI.Idea_Number=GWP.WT_Project_Number 
			...
			SELECT @vMsg_Out='GDI Details Added Successfully'
	COMMIT TRAN
	RETURN 1
 ...
 ERR_HANDLER:
	BEGIN
		SELECT @vMsg_Out='Failed To Add GDI Details-'+ ERROR_MESSAGE();
		IF (@@TRANCOUNT>0)
		ROLLBACK TRAN
		RETURN 0
	END
...
/****** Object:  StoredProcedure [dbo].[Sp_UpdWTGDIDetails]    Script Date: 6/29/2019 12:41:27 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[Sp_UpdWTGDIDetails]
(
	@vWT_Project_Id int,
	@vGDI_Number varchar(15),
 	@vGDI_Name nvarchar(500),
	@vPlan_Start_Date datetime,
	@vProject_Summary nvarchar(4000),
	@vProject_Metric_Cp nvarchar(4000),
...
IF (@vGDI_Number IS NULL OR LEN(LTRIM(RTRIM(@vGDI_Number)))<1)
	BEGIN
...
SELECT @_vGDI_Id=B.GDI_Id  
FROM GPM_WT_Project A INNER JOIN GPM_WT_GDI B ON A.WT_Id=B.GDI_Id AND A.WT_Project_Number=B.GDI_Number  
WHERE A.WT_Code=@_vWT_Type AND 
A.WT_Project_ID=@vWT_Project_Id AND
A.WT_Project_Number=LTRIM(RTRIM(@vGDI_Number)) 
...
BEGIN TRAN
	...
	UPDATE GPM_WT_GDI
		...
		WHERE GDI_Id=@_vGDI_Id
			 ...
			 IF (@@ERROR <> 0) GOTO ERR_HANDLER
					...
					BEGIN
						...
						DELETE FROM GPM_WT_GDI_MS_Attrib  WHERE GDI_Id=@_vGDI_Id AND Piller_Id IS NOT NULL
			...
			/*Delete All piller ids if select list is blank*/
				DELETE FROM GPM_WT_GDI_MS_Attrib  WHERE GDI_Id=@_vGDI_Id
			...
			/* Add Remove Team Members According to new member list*/
						...
						/* Disable Role User List if it is already exist and not in new list */
							...
							/* Send Notification */
								...
								GD_User_Id,
								@vLast_Modified_By,
								'You have been assigned as '+ GPTR.WT_Role_Name +' for project '+ @vGDI_Name,
								@vGDI_Name,
								GETDATE(),
								'N'
		...
		SELECT @vMsg_Out='GDI Details Updated Successfully'	
...
RETURN 1
 ...
 ERR_HANDLER:
	BEGIN
			SELECT @vMsg_Out='Failed To Update GDI Details-'+ ERROR_MESSAGE();
		IF (@@TRANCOUNT>0)
		ROLLBACK TRAN
		RETURN 0
	END
