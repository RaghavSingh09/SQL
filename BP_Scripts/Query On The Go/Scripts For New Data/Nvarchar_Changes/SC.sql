ALTER TABLE GPM_WT_Supply_Chain ALTER COLUMN SC_Name NVARCHAR(500)
GO
ALTER TABLE GPM_WT_Supply_Chain ALTER COLUMN Project_Description NVARCHAR(4000)
GO
ALTER TABLE GPM_WT_Supply_Chain ALTER COLUMN Est_Timeline NVARCHAR(4000)
GO

/****** Object:  StoredProcedure [dbo].[Sp_AddWTSCDetails]    Script Date: 6/29/2019 1:30:29 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[Sp_AddWTSCDetails]
(
	@vSC_Name nvarchar(500),
...
DECLARE @_vSC_Number VARCHAR(15)=NULL
DECLARE @_vSC_Number_SEQ INT =NULL
DECLARE @_vSC_Id INT
DECLARE @_vWT_Type Varchar(5)='SC'
...
IF NOT EXISTS(SELECT * FROM GPM_WT_Supply_Chain)
       SELECT @_vSC_Number=@_vWT_Type+'-00001'
ELSE
BEGIN
       SELECT @_vSC_Number_SEQ=MAX(CAST(SUBSTRING(SC_Number,CHARINDEX('-',SC_Number,1)+1,LEN(SC_Number)) AS INT))
FROM GPM_WT_Supply_Chain WHERE CHARINDEX('-',SC_Number,1)>0 AND UPPER(SUBSTRING(SC_Number,1, CHARINDEX('-',SC_Number,1)-1))=UPPER(@_vWT_Type)
AND ISNUMERIC(SUBSTRING(SC_Number,CHARINDEX('-',SC_Number,1)+1,LEN(SC_Number)) )=1

       IF(@_vSC_Number_SEQ IS NULL)
              SELECT @_vSC_Number=@_vWT_Type+'-00001'
       ELSE
              BEGIN
                     SELECT @_vSC_Number_SEQ=@_vSC_Number_SEQ+1

                     IF(@_vSC_Number_SEQ>99999)
                    BEGIN
						SELECT @vMsg_Out = 'Sequence Reached To Maximum Limit'
						RETURN 0
					 END
                     ELSE
                           SELECT @_vSC_Number= @_vWT_Type+'-'+REPLICATE('0', 5-LEN(CAST(@_vSC_Number_SEQ AS VARCHAR(10))) ) + CAST(@_vSC_Number_SEQ AS VARCHAR(10))
              END
...
END
 ...
 BEGIN TRAN
											...
											WHILE @_vDvCnt<(@_DvCntMax+1)
												BEGIN
													...
													VALUES	(	@vWT_Project_Id,
																...
																@_vStartDate,
																@_vEndDate,
																CASE WHEN @_vDeliverable_Id IN(257,258,270) THEN 'Y' ELSE 'N' END, 
																NULL, 
																@vCreated_By, 
																GETDATE(), 
																@vLast_Modified_By, 
			...
			WHERE WT_Project_Id=@vWT_Project_Id 
					...
					/* Add Team Members*/
						...
						IF(LEN(@vSponser)>0)
							INSERT INTO @_vTabRoleMember(RoleMember) Values(@vSponser)
			...
			*/
					...
					BEGIN
						INSERT INTO GPM_WT_Project_Notification
								...
								SELECT DISTINCT @vWT_Project_Id,
									...
									GWPT.GD_User_Id,
									@vLast_Modified_By,
									'IDEA '+GWI.Idea_Name+' has been converted into Project '+@vSC_Name,
									@vSC_Name,
									GETDATE(),
									'N'
									FROM GPM_WT_Idea GWI INNER JOIN GPM_WT_Project GWP On GWI.Idea_Id=GWP.WT_Id AND GWI.Idea_Number=GWP.WT_Project_Number 
			...
			SELECT @vMsg_Out='Supply Chain Details Added Successfully'
	COMMIT TRAN
	RETURN 1
 ...
 ERR_HANDLER:
	BEGIN
		SELECT @vMsg_Out='Failed To Add Supply Chain -'+ ERROR_MESSAGE();
		IF (@@TRANCOUNT>0)
		ROLLBACK TRAN
		RETURN 0
	END
...
/****** Object:  StoredProcedure [dbo].[Sp_UpdWTSCDetails]    Script Date: 6/29/2019 1:30:54 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[Sp_UpdWTSCDetails]
(
	@vWT_Project_Id int,
	@vSC_Number Varchar(15),
 	@vSC_Name nvarchar(500),
	@vPlan_Start_Date datetime,
	@vProj_Track_Id int,
	@vProj_Status_Id int,
...
IF (@vSC_Number IS NULL OR LEN(LTRIM(RTRIM(@vSC_Number)))<1)
	BEGIN
...
SELECT @_vSC_Id=B.SC_Id  
FROM GPM_WT_Project A INNER JOIN GPM_WT_Supply_Chain B ON A.WT_Id=B.SC_Id AND A.WT_Project_Number=B.SC_Number  
WHERE A.WT_Code=@_vWT_Type AND 
A.WT_Project_ID=@vWT_Project_Id AND
A.WT_Project_Number=LTRIM(RTRIM(@vSC_Number))
...
BEGIN TRAN
	...
	UPDATE GPM_WT_Supply_Chain
		SET SC_Name = @vSC_Name,
		...
		/* Add Remove Team Members According to new member list*/
						...
						/* Disable Role User List if it is already exist and not in new list */
							...
							INSERT INTO GPM_WT_Project_Notification
								...
								GD_User_Id,
								@vLast_Modified_By,
								'You have been assigned as '+ GPTR.WT_Role_Name +' for project '+ @vSC_Name,
								@vSC_Name,
								GETDATE(),
								'N'
		...
		SELECT @vMsg_Out='Supply Chain Details Updated Successfully'	
...
RETURN 1
 ...
 ERR_HANDLER:
	BEGIN
			SELECT @vMsg_Out='Failed To Update Supply Chain Details-'+ ERROR_MESSAGE();
		IF (@@TRANCOUNT>0)
		ROLLBACK TRAN
		RETURN 0
	END
