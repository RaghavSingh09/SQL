--USE [PMT_Migration]
--GO
--/****** Object:  StoredProcedure [dbo].[Temp_SC_DATA_LOAD]    Script Date: 4/3/2019 7:00:35 PM ******/
--SET ANSI_NULLS ON
--GO
--SET QUOTED_IDENTIFIER ON
--GO
--ALTER PROCEDURE [dbo].[Temp_SC_DATA_LOAD]
--AS
--Below variable for SC Coustom Field
...
-- Add 6 new TAGS for SC role&tags without repetition value
...
-- Add 15 new TAGS for SC role&tags without repetition value.(TAGS),,
...
DECLARE @_vGlobal_SC_Type VARCHAR(2000)
DECLARE @_vSC_Initiative VARCHAR(2000)

--Add Primari key for SC.. 
DECLARE @_vBA_Id INT
...
--DECLARE @_vBP_Score_Type_Code VARCHAR(20) not in SC
...
DECLARE @_vGlobal_SC_Type_Id INT
DECLARE @_vSC_Initiative_Id INT
...
TRUNCATE TABLE Temp_SC_Error

TRUNCATE TABLE Temp_SC_MissingRoleMember_Error

TRUNCATE TABLE Temp_SC_Gate_Deliverable_Error
...
DELETE FROM GPM_WT_Supply_Chain

SELECT @_vDBGateCnt=COUNT(*) FROM GPM_Gate_WT_Map WHERE WT_Code='SC'  AND Is_Deleted_Ind='N'
...
DECLARE SC_Cursor CURSOR FOR
	...
	FROM
		Temp_SC_Custom_Fields 
		--WHERE Sequence_number IN('SC-39741','SC-38941')

	OPEN SC_Cursor
	FETCH NEXT FROM SC_Cursor INTO 
			@_vName,  	
			@_vPowerSteering_ID,	
			@_vSequence_number,
			@_vProject_Lead,
		...
		WHILE @@FETCH_STATUS = 0
			...
			SELECT 
				...
				SELECT @_vProject_Tracking_Indicator_Id=Proj_Track_Id FROM GPM_Project_Tracking WHERE RTRIM(LTRIM(Proj_Track_Status))=(CASE WHEN RTRIM(LTRIM(@_vProject_Tracking_Indicator))='Canceled' Then 'Cancelled' Else LTRIM(RTRIM(@_vProject_Tracking_Indicator)) END)
				...
				BEGIN
					SELECT @_vError_Desc += '| MA : SC Name Is Blank'
				...
				IF(@_vProject_Lead IS NULL OR LEN(LTRIM(RTRIM(@_vProject_Lead)))<=0)
					SELECT @_vProject_Lead = LTRIM(RTRIM(TDR.Project_Lead)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
				...
				SELECT @_vFinancial_Rep=LTRIM(RTRIM(TDR.Financial_Rep)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
				...
				SELECT @_vSponsor=LTRIM(RTRIM(TDR.Sponsor)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
				...
				IF EXISTS(
					SELECT COUNT(*) FROM Temp_SC_Gate_Deliverable  WHERE RTRIM(LTRIM(Sequence_number))=RTRIM(LTRIM(@_vSequence_number))
				AND Work_type='Gate'
				...
				BEGIN
						...
						SELECT @_vTempGateCnt=COUNT(*) FROM Temp_SC_Gate_Deliverable WHERE Sequence_number=RTRIM(LTRIM(@_vSequence_number)) AND Work_type='GATE'
							...
							END
							ELSE
							IF EXISTS(SELECT 1 FROM Temp_SC_Gate_Deliverable TGGD WHERE TGGD.Sequence_number=RTRIM(LTRIM(@_vSequence_number)) AND TGGD.Work_type='GATE'
										AND NOT EXISTS( SELECT 1 FROM GPM_Gate_WT_Map GGWM INNER JOIN GPM_Gate GG On GGWM.Gate_Id=GG.Gate_Id
													WHERE GGWM.WT_Code='SC' AND GG.Alt_Gate_Desc=TGGD.Gate_Name))
							BEGIN
								SELECT @_vError_Desc += '| One or more gate not found in given data'
								SELECT @_vValidData='FALSE'
							END
				...
				/* Non Mandatory Attribute Validation */
				IF(LEN(LTRIM(RTRIM(@_vProject_Tracking_Indicator)))>0)
				BEGIN
					IF(@_vProject_Tracking_Indicator_Id IS NULL OR  @_vProject_Tracking_Indicator_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vProject_Tracking_Indicator +' Project Tracking Indicator Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

				IF(LEN(LTRIM(RTRIM(@_vStatus)))>0)
				BEGIN
					IF(@_vStatus_Id IS NULL OR  @_vStatus_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vStatus +' Status Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

				IF(LEN(LTRIM(RTRIM(@_vSpend_Type)))>0)
				BEGIN
					IF(@_vSpend_Type_Id IS NULL OR  @_vSpend_Type_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vSpend_Type +' Spend Type Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

				IF(LEN(LTRIM(RTRIM(@_vProject_Type)))>0)
				BEGIN
					IF(@_vProj_Type_Id IS NULL OR  @_vProj_Type_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vProject_Type +' Project Type Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

			
				IF(LEN(LTRIM(RTRIM(@_vRate_or_Non_Rate)))>0)
				BEGIN
					IF(@_vRate_Type_Id IS NULL OR  @_vRate_Type_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vRate_or_Non_Rate +' Rate or Non_Rate Type Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END
				...
				BEGIN
					...
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vFinancial_Impact_Area +' Financial Impact Area Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

				IF(LEN(LTRIM(RTRIM(@_vGPH_Category)))>0)
				BEGIN
					IF(@_vGPH_Cat_Id IS NULL OR  @_vGPH_Cat_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vGPH_Category +' GPH Category Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END

				IF(LEN(LTRIM(RTRIM(@_vGPH_Sub_Category)))>0)
				BEGIN
					IF(@_vGPH_Sub_Cat_Id IS NULL OR  @_vGPH_Sub_Cat_Id<=0)
					BEGIN
						SELECT @_vError_Desc += '| NMA :'+ @_vGPH_Sub_Category +' GPH Sub Category Not Found in DB Master Data'
						SELECT @_vValidData='FALSE'
					END
				END
				...
				IF EXISTS(
					SELECT *
					FROM Temp_SC_Gate_Deliverable TPGD WHERE TPGD.Sequence_number=@_vSequence_number AND TPGD.Work_Type='Gate'
					...
					WHERE GGWM.WT_Code='SC' AND GG.Alt_Gate_Desc=TPGD.Gate_Name) !=(SELECT COUNT(*)
					FROM Temp_SC_Gate_Deliverable TPGD1 WHERE TPGD1.Sequence_number=@_vSequence_number AND TPGD1.Work_Type='Deliverable'
				...
				*/
				...
				ELSE
				BEGIN
										-- Use Add_SP for insert value into GPM_WT_SC or SC...now SC
												-- Error came here..after full running
				
				INSERT INTO GPM_WT_Supply_Chain
					(
						SC_Number,
						SC_Name,
						Plan_Start_Date,
					...
					(
						...
						@_vCountry_Code,
						@_vLocation_ID,
						CAST(CASE WHEN LEN(LTRIM(RTRIM(@_vFull_Potential_Savings_$)))>0 THEN
							REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@_vFull_Potential_Savings_$,'$',''),',','') ,')',''),'(','-'),' ','')
						ELSE NULL END AS NUMERIC),

						CASE WHEN @_vProbability_of_Achieving_Savings='' THEN NULL ELSE @_vProbability_of_Achieving_Savings END,

						CAST(CASE WHEN LEN(LTRIM(RTRIM(@_vEstimated_Baseline_Savings_$)))>0 THEN
			...
			-- Start for Gate & Deliverable	
					...
					SELECT @_vActive_Gate=LTRIM(RTRIM(Active_Gate)) FROM Temp_SC_Gate_Deliverable WHERE RTRIM(LTRIM(Sequence_number))=RTRIM(LTRIM(@_vSequence_number)) AND Active_Gate IS NOT NULL
					--			
					...
					SELECT @_vWT_Project_Id, 
							...
							@_vCreated_By,
							Getdate()
					FROM Temp_SC_Gate_Deliverable TDGA INNER JOIN GPM_Gate GG On RTRIM(LTRIM(TDGA.Gate_Name))=RTRIM(LTRIM(GG.Alt_Gate_Desc))
					INNER JOIN GPM_Gate_WT_Map GGWM On GG.Gate_Id=GGWM.Gate_Id WHERE TDGA.Sequence_number = RTRIM(LTRIM(@_vSequence_number))
					AND GGWM.WT_Code='SC' AND Deliverable_Name IS NULL
			...
			--Start for Deliverable
												INSERT INTO GPM_WT_Project_Deliverable 
													...
													SELECT 
															...
															@_vCreated_By,
															TDGD.System_end_date
															FROM Temp_SC_Gate_Deliverable TDGD 
													...
													SELECT 
														@_vWT_Project_Id,
														(SELECT WT_Role_Id FROM GPM_Project_Template_Role where WT_Code= 'SC' AND WT_Role_Name = 'Deliverable Leader' AND Is_Deleted_Ind='N'),
														@_vGate_Id,
														GGD.Deliverable_Id,
	...
	/*							INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									SELECT 'Project Lead',LTRIM(RTRIM(TDR.Project_Lead)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
*/
								...
								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									...
									/*
								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									SELECT 'Sponsor',LTRIM(RTRIM(TDR.Sponsor)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 

									
								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									SELECT 'Financial Rep',LTRIM(RTRIM(TDR.Financial_Rep)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
									*/
								...
								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									...
									(
									SELECT LTRIM(RTRIM(REPLACE(TDR.Team_Member, CHAR(10),'|'))) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number)
									) TAB 

								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									SELECT 'Managers',LTRIM(RTRIM(TDR.Manager)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 

								INSERT INTO @_vProjectMember_Table(WT_Role_Name,Project_Member_Name)
									SELECT 'Stakeholders',LTRIM(RTRIM(TDR.Stakeholder)) FROM Temp_SC_RoleAndTags TDR WHERE RTRIM(LTRIM(TDR.Sequence_number))= @_vSequence_number 
										...
										BEGIN
											...
											IF(@_vProject_Member_Id IS NOT NULL AND LEN(@_vProject_Member_Id)>0)
												BEGIN
													...
													(
													@_vWT_Project_Id,
													(SELECT WT_Role_Id FROM GPM_Project_Template_Role WHERE WT_Code='SC' AND WT_Role_Name=@_vWT_Role_Name),
													@_vProject_Member_Id,
													'N',
													@_vCreated_By,
													@_vSystem_start_date,
											...
											INSERT INTO Temp_SC_MissingRoleMember_Error
												(
													Name,
													PowerSteering_ID,
													Sequence_number,
			...
			/*
						...
						*/
						...
						@_vCreated_By,
						@_vSystem_end_date
					FROM Temp_SC_TDC TDC INNER JOIN GPM_Metrics_TDC_Saving GMTS On LTRIM(RTRIM(TDC.TDC_Attrib))=GMTS.Attrib_Name
...
-- Again use Curser..
	...
	FETCH NEXT FROM SC_Cursor INTO 
	 ...
	 IF CURSOR_STATUS('global','SC_Cursor')>=-1
	 BEGIN
       DEALLOCATE SC_Cursor
	 END
 END
